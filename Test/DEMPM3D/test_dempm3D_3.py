...
if __name__ == "__main__":
    ...
    dem = DEM.DEM(cmType=0,                                                 # /Contact model type/ 0 for linear model; 1 for hertz model; 2 for linear rolling model; 3 for linear bond model
                  domain=ti.Vector([5, 5, 6]),                              # domain size
    ...
    dem.MatInfo[0].localDamping = [0.2, 0.2]                                # /Local damping/ Translation & Rolling
    ...
    dem.BodyInfo[0].pos0 = ti.Vector([2.5, 2.5, 3.75])                         # Initial position or center of sphere of Body
    dem.BodyInfo[0].rhi = 0.75                                              # Particle radius
    dem.BodyInfo[0].rlo = 0.75                                              # Particle radius
    ...
    # MPM domain
    mpm = MPM.MPM(domain=ti.Vector([5, 5, 6]),                              # domain size
                  dx=ti.Vector([0.1, 0.1, 0.1]),                            # size of one grid
    ...
    mpm.BodyInfo[0].len = ti.Vector([5., 5., 3.])                           # Size of Body
    ...
    for i, j, k in ti.ndrange((0, 51), (0, 51), (0, 2)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, 0, -1]))
    for i, j, k in ti.ndrange((0, 51), (0, 51), (59, 61)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, 0, 1]))
    for i, j, k in ti.ndrange((0, 51), (0, 2), (0, 61)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, -1, 0]))
    for i, j, k in ti.ndrange((0, 51), (49, 51), (0, 61)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, 1, 0]))
    for i, j, k in ti.ndrange((0, 2), (0, 51), (0, 61)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([-1, 0, 0]))
    for i, j, k in ti.ndrange((49, 51), (0, 51), (0, 61)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([1, 0, 0]))
