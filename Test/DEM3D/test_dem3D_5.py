...
if __name__ == "__main__":
    ...
    dem.BodyInfo[0].pos0 = ti.Vector([0.125, 0., 0.05])                        # Initial position or center of sphere of Body
    dem.BodyInfo[0].len = ti.Vector([0.05, 0.03, 0.5])                     # Initial position or center of sphere of Body
    dem.BodyInfo[0].rhi = 0.001836                                          # Particle radius
    dem.BodyInfo[0].rlo = 0.001836                                          # Particle radius
    dem.BodyInfo[0].pnum = 8000                                              # The number of particle in the group
    ...
    # Create MPM domain
    dem.AddContactModel()
    dem.AddBodies(max_particle_num=8000)
    dem.AddWall(max_facet_num=5)
    dem.AddContactPair(max_contact_num=24000)
    dem.AddNeighborList(multiplier=3.5, max_potential_particle_pairs=150000, max_potential_wall_pairs=50000)
