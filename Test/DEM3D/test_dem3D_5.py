...
import math
ti.init(arch=ti.cpu, default_fp=ti.f32, debug=False, kernel_profiler=True)


# Compare to LIGGGHTS
if __name__ == "__main__":
    # DEM domain
    dem = DEM.DEM(cmType=1,                                                 # /Contact model type/ 0 for linear model; 1 for hertz model; 2 for linear rolling model; 3 for linear bond model
                  domain=ti.Vector([0.31, 0.03, 1.0]),                     # domain size
                  ...
                  wallNum=6,                                                # Taichi field of material parameters *Number of Wall*
    ...
    dem.MatInfo[0].Modulus = 5e6                                                 # Contact normal stiffness
    dem.MatInfo[0].possion = 0.3                                                 # Contact tangential stiffness
    ...
    dem.MatInfo[0].Restitution = 0.3                               # /Viscous damping/ 
    ...
    dem.BodyInfo[0].pos0 = ti.Vector([0., 0., 0.05])                        # Initial position or center of sphere of Body
    dem.BodyInfo[0].len = ti.Vector([0.31, 0.03, 0.15])                     # Initial position or center of sphere of Body
    dem.BodyInfo[0].rhi = 0.001836                                          # Particle radius
    dem.BodyInfo[0].rlo = 0.001836                                          # Particle radius
    dem.BodyInfo[0].pnum = 9490                                              # The number of particle in the group
    ...
    dem.WallInfo[1].point3 = ti.Vector([0., 0.03, 1.0])                    # The vertex of the wall
    dem.WallInfo[1].point4 = ti.Vector([0., 0., 1.0])                      # The vertex of the wall
    ...
    dem.WallInfo[2].point3 = ti.Vector([0.31, 0.03, 1.0])                  # The vertex of the wall
    dem.WallInfo[2].point4 = ti.Vector([0.31, 0., 1.0])                    # The vertex of the wall
    ...
    dem.WallInfo[3].point3 = ti.Vector([0.31, 0., 1.0])                    # The vertex of the wall
    dem.WallInfo[3].point4 = ti.Vector([0., 0., 1.0])                      # The vertex of the wall
    ...
    dem.WallInfo[4].point3 = ti.Vector([0.31, 0.03, 1.0])                  # The vertex of the wall
    dem.WallInfo[4].point4 = ti.Vector([0., 0.03, 1.0])                    # The vertex of the wall
    ...
    dem.WallInfo[5].point1 = ti.Vector([0., 0., 1.0])                      # The vertex of the wall
    dem.WallInfo[5].point2 = ti.Vector([0.31, 0., 1.0])                    # The vertex of the wall
    dem.WallInfo[5].point3 = ti.Vector([0.31, 0.03, 1.0])                  # The vertex of the wall
    dem.WallInfo[5].point4 = ti.Vector([0., 0.03, 1.0])                    # The vertex of the wall
    dem.WallInfo[5].norm = ti.Vector([0, 0, -1])                            # The norm of the wall

   
    # Create MPM domain
    dem.AddContactModel()
    dem.AddBodies(max_particle_num=10000)
    dem.AddWall(max_facet_num=6)
    dem.AddContactPair(max_contact_num=100000)
    dem.AddNeighborList(multiplier=3.5, max_potential_particle_pairs=700000, max_potential_wall_pairs=50000)

    # Solve
    TIME: float = 1.0                                                         # Total simulation time
    saveTime: float = 5e-2                                                  # save per time step
