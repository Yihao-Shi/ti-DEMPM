...
def InitialStress(mpm: ti.template()):
    for np in range(mpm.lp.particleNum[None]):
        ...
        mpm.lp.stress[np][0, 0] = mpm.lp.stress[np][1, 1] = mpm.lp.stress[np][2, 2] = mpm.lp.P[np]


# Test for Newtonian models using GIMP
if __name__ == "__main__":
    # MPM domain
    mpm = MPM.MPM(domain=ti.Vector([1.66, 0.65, 0.7]),                           # domain size
                  ...
                  max_particle_num=1.1e5,                                     # The max number of particles
    ...
    mpm.BodyInfo[0].pos0 = ti.Vector([0.05, 0.05, 0.05])                          # Initial position or center of sphere of Body
    mpm.BodyInfo[0].len = ti.Vector([0.6, 0.3, 0.6])                             # Size of Body
    ...
    for i, j, k in ti.ndrange((0, 167), (0, 66), (4, 6)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, 0, -1]))
    for i, j, k in ti.ndrange((0, 167), (0, 66), (70, 71)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, 0, 1]))
    for i, j, k in ti.ndrange((4, 6), (0, 66), (0, 71)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([-1, 0, 0]))
    for i, j, k in ti.ndrange((165, 167), (0, 66), (0, 71)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([1, 0, 0]))
    for i, j, k in ti.ndrange((0, 167), (4, 6), (0, 71)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, -1, 0]))
    for i, j, k in ti.ndrange((0, 167), (35, 37), (0, 71)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, 1, 0]))
