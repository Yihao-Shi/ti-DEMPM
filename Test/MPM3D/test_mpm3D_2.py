...
def InitialStress(mpm: ti.template()):
    for np in range(mpm.lp.particleNum[None]):
        if 10 < mpm.lp.x[np][0] < 40:
            mpm.lp.stress[np][2, 2] = -(mpm.lp.x[np][2] - 45) * mpm.Gravity[2] * mpm.BodyInfo[0].MacroRho
            mpm.lp.stress[np][0, 0] = mpm.lp.stress[np][1, 1] = mpm.lp.stress[np][2, 2] * mpm.MatInfo[0].mu / (1 - mpm.MatInfo[0].mu)
        elif 40 < mpm.lp.x[np][0] < 60:
            mpm.lp.stress[np][2, 2] = -(mpm.lp.x[np][2] - (-mpm.lp.x[np][2] + 105)) * mpm.Gravity[2] * mpm.BodyInfo[0].MacroRho
            mpm.lp.stress[np][0, 0] = mpm.lp.stress[np][1, 1] = mpm.lp.stress[np][2, 2] * mpm.MatInfo[0].mu / (1 - mpm.MatInfo[0].mu)
        elif mpm.lp.x[np][0] > 60:
            mpm.lp.stress[np][2, 2] = -(mpm.lp.x[np][2] - 25) * mpm.Gravity[2] * mpm.BodyInfo[0].MacroRho
            mpm.lp.stress[np][0, 0] = mpm.lp.stress[np][1, 1] = mpm.lp.stress[np][2, 2] * mpm.MatInfo[0].mu / (1 - mpm.MatInfo[0].mu)
...
if __name__ == "__main__":
    # MPM domain
    mpm = MPM.MPM(domain=ti.Vector([130, 50, 50]),                              # domain size
                  dx=ti.Vector([0.5, 0.5, 0.5]),                                 # size of one grid
                  ...
                  max_particle_num=1.5e5,                                     # The max number of particles
    ...
    mpm.BodyInfo[0].pos0 = ti.Vector([10, 10, 10])                              # Initial position or center of sphere of Body
    mpm.BodyInfo[0].len = ti.Vector([110, 7, 15])                              # Size of Body
    ...
    mpm.BodyInfo[1].pos0 = ti.Vector([40, 10, 25])                              # Initial position or center of sphere of Body
    mpm.BodyInfo[1].len = ti.Vector([20, 7, 20])                               # Size of Body
    mpm.BodyInfo[1].v0 = ti.Vector([0, 0, 0])                                  # Initial velocity
    mpm.BodyInfo[1].fixedV = ti.Vector([0, 0, 0])                              # Fixed velocity
    ...
    mpm.BodyInfo[2].pos0 = ti.Vector([10, 10, 25])                              # Initial position or center of sphere of Body
    mpm.BodyInfo[2].len = ti.Vector([30, 7, 20])                               # Size of Body
    mpm.BodyInfo[2].v0 = ti.Vector([0, 0, 0])                                  # Initial velocity
    mpm.BodyInfo[2].fixedV = ti.Vector([0, 0, 0])                              # Fixed velocity
    ...
    for i, j, k in ti.ndrange((0, 261), (0, 101), (19, 21)):
        Grid.SetNonSlippingBC(i, j, k)
    for i, j, k in ti.ndrange((19, 21), (0, 101), (0, 101)):
        Grid.SetNonSlippingBC(i, j, k)
    for i, j, k in ti.ndrange((239, 241), (0, 101), (0, 101)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([1, 0, 0]))
    for i, j, k in ti.ndrange((0, 261), (19, 21), (0, 101)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, -1, 0]))
    for i, j, k in ti.ndrange((0, 261), (34, 36), (0, 101)):
        Grid.SetSlippingBC(i, j, k, ti.Vector([0, 1, 0]))
