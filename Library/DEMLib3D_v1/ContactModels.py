...
class ContactModel:
    ...
    def DEMPMBarrierMethod(self):
        self.scalar = ti.field(float, shape=(max_material_num,))
        self.slipLim = ti.field(float, shape=(max_material_num,))
...
class LinearContactModel(ContactModel):
    ...
    def ComputeContactTangentialForce(self, ContactPair, nc, matID1, matID2, m_eff, v_rel, particle_num):
        ...
        key  = ContactPair.HashValue(ContactPair.TYPE[nc] * particle_num + ContactPair.endID1[nc], ContactPair.endID2[nc])
...
class HertzMindlinContactModel(ContactModel):
    ...
    def DampingRatio(self, matID1, matID2):
        ...
        if restitution == 0:
            ...
            res = ti.log(res / ti.sqrt(math.pi * math.pi + ti.log(res) * ti.log(res)))
        return res
...
class LinearRollingResistanceContactModel(LinearContactModel):
    ...
    def ComputeRollingFriction(self, ContactPair, nc, matID1, matID2, w1, w2, m_eff, rad_eff, particle_num):
        vr = -rad_eff * (w1 - w2).cross(ContactPair.norm[nc])
        ks = EffectiveValue(self.ks[matID1], self.ks[matID2])
        kr =  0.5 * ks * rad_eff * rad_eff 
        ...
        if trial_fr.norm() > fricRoll:
            ContactPair.Tr[nc] = rad_eff * fricRoll * ContactPair.norm[nc].cross(Normalize(vr))
        else:
            ContactPair.Tr[nc] = rad_eff * trial_fr.norm() * ContactPair.norm[nc].cross(Normalize(vr))
    ...
    def ComputeTorsionFriction(self, ContactPair, nc, matID1, matID2, w1, w2, m_eff, rad_eff, particle_num):
        vt = (w1 - w2).dot(ContactPair.norm[nc])
        ks = EffectiveValue(self.ks[matID1], self.ks[matID2])
        kt =  0.5 * ks * ContactPair.gapn[nc] * rad_eff 
        trial_ft = -kt * vt * self.dt
        key  = ContactPair.HashValue(ContactPair.TYPE[nc] * particle_num + ContactPair.endID1[nc], ContactPair.endID2[nc])
        for i in range(ContactPair.contactNum0[None]):
            if ContactPair.key[i] == key:
                trial_ft = trial_ft + ContactPair.RelTwist[i]
        
        tmiu = ti.min(self.TMu[matID1], self.TMu[matID2])
        miu = ti.min(self.Mu[matID1], self.Mu[matID2])
        fricTwist = 2./3. * tmiu * miu * rad_eff * ContactPair.cnforce[nc].norm()
        if trial_ft > fricTwist:
            ContactPair.Tt[nc] = fricTwist * ContactPair.norm[nc]
        else:
            ContactPair.Tt[nc] = trial_ft * ContactPair.norm[nc]
